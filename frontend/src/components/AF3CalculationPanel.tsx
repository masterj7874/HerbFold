...
const dateLabel = (value: string) => { const date = new Date(value); return Number.isNaN(date.valueOf()) ? value : date.toLocaleString("ko-KR", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit" }); };
...
export default function AF3CalculationPanel({ compound, targetAccession, targetName, preferredJobId, refreshKey = 0, onResult, onJobsChanged, onArchive }: {
  ...
  return <section className="afc-panel" data-testid="af3-calculation-panel" aria-busy={loading}>
    <header className="afc-heading"><div><span className="afc-eyebrow">선택 성분별 실제 구조 계산</span><h2 tabIndex={-1} data-testid="af3-calculation-heading">{compound.name_ko || compound.name} × {targetName ? `${targetName} · ` : targetAccession === "P35354" ? "COX-2 · " : ""}{targetAccession}</h2><p>준비 → 실행 → 진행 확인 → 해당 작업의 예측 구조. 결합 친화도·약효는 별도 검증 대상입니다.</p></div><div className="afc-header-actions"><button className="afc-outline" onClick={() => setListRevision((value) => value + 1)} disabled={loading || !!busy} data-testid="studio-af3-list-refresh"><RefreshCw size={16} />작업 새로고침</button><button className="afc-outline" onClick={onArchive}><FolderOpen size={16} />연구 기록</button></div></header>
    {loading ? <p className="afc-loading"><LoaderCircle size={18} className="spin" />이 성분·표적의 저장 작업을 연결하는 중</p> : <>
      <div className="afc-layout">
        <div className="afc-inputs">
          <h3>1. 계산 조건과 입력 준비</h3>
          <label className="afc-field">계산 모드<select value={mode} onChange={(event) => { setMode(event.target.value as PredictionMode); setAcknowledged(false); }} disabled={!!busy} data-testid="studio-af3-mode"><option value="search">표준 · MSA 검색</option><option value="none">탐색 · MSA·템플릿 없음</option></select></label>
          <p className="afc-mode-scope">{mode === "search" ? "진화 정보와 템플릿 검색을 포함합니다. 공식 검색 데이터베이스와 실행 환경이 필요합니다." : "진화 정보와 템플릿을 생략하는 탐색 계산입니다. 낮은 신뢰도와 왜곡된 구조가 나올 수 있어 표준 결과와 구분합니다."}</p>
          {mode === "none" && <label className="afc-ack"><input type="checkbox" checked={acknowledged} onChange={(event) => setAcknowledged(event.target.checked)} data-testid="studio-af3-exploratory-ack" /><span>MSA·템플릿 생략과 정확도 제한을 확인했으며 탐색 계산을 선택합니다.</span></label>}
          <label className="afc-field afc-seed">재현용 시드<input type="number" min="0" max="4294967295" step="1" value={seed} onChange={(event) => setSeed(event.target.value)} disabled={!!busy} data-testid="studio-af3-seed" /></label>
          <button className="afc-primary" disabled={!!busy || !!refreshRequired || (mode === "none" && !acknowledged)} onClick={() => void prepare()} data-testid="studio-af3-prepare">{busy === "prepare" ? <LoaderCircle size={17} className="spin" /> : <FileCheck2 size={17} />}{busy === "prepare" ? "입력 준비 중" : "이 조건으로 입력 준비"}</button>
          <small className="afc-muted">실행 버튼을 누르면 선택 성분의 실제 AF3 작업을 제출합니다.</small>
        </div>
        <div className="afc-job" data-testid="studio-af3-job" data-job-id={active?.job.id || ""} data-job-status={active?.job.status || "none"}>
          <div className="afc-job-heading"><h3>2. 실행과 결과</h3>{active && <button className="afc-icon" aria-label="선택 AF3 작업 새로고침" onClick={() => setRefreshRevision((value) => value + 1)} data-testid="studio-af3-refresh"><RefreshCw size={17} /></button>}</div>
          {entries.length > 1 && <label className="afc-field">이 성분의 계산 기록<select value={selectedId || ""} disabled={!!busy} onChange={(event) => { setSelectedId(event.target.value); setLog(null); setNotice(null); }} data-testid="studio-af3-job-select">{entries.map((entry) => <option key={entry.job.id} value={entry.job.id}>{dateLabel(entry.job.created)} · {modeLabel(entry.requested.msa_mode)} · {labels[entry.job.status] || entry.job.status}</option>)}</select></label>}
          {!active ? <div className="afc-empty"><FileCheck2 size={28} /><p>{error ? "저장 작업을 확인하지 못했습니다. 작업 새로고침으로 다시 조회하세요." : <>이 성분과 표적의 계산 기록이 없습니다.<br />입력을 준비하면 실행 가능 여부를 확인할 수 있습니다.</>}</p></div> : <>
            <div className="afc-status" data-status={invalidOutput ? "failed" : active.job.status} role="status">{live && !statusError ? <Cpu size={17} /> : statusError ? <CircleAlert size={17} /> : invalidOutput ? <CircleAlert size={17} /> : active.job.status === "completed" ? <Check size={17} /> : RETRY_STATUSES.has(active.job.status) ? <CircleAlert size={17} /> : <FileCheck2 size={17} />}<strong>{statusError ? `연결 재확인 중 · 마지막 상태: ${labels[active.job.status] || active.job.status}` : invalidOutput ? "계산 종료 · 출력 검증 실패" : labels[active.job.status] || active.job.status}</strong>{active.queue_position != null && <span>대기 순번 {active.queue_position}</span>}</div>
            <p className="afc-job-scope" data-testid="studio-af3-job-mode">{modeLabel(active.requested.msa_mode)} · 시드 {active.requested.seeds.join(", ")}</p>
            <dl className="afc-job-facts"><div><dt>작업 ID</dt><dd>{active.job.id}</dd></div><div><dt>마지막 갱신</dt><dd>{dateLabel(active.job.updated)}</dd></div></dl>
            {active.requested.msa_mode === "none" && <p className="afc-caution">탐색용 예측입니다. 계산 완료 자체가 구조 정확도나 결합을 입증하지 않습니다.</p>}
            {parameterStatus === "test_parameters" && !blockers.includes(TEST_WEIGHT_BLOCKER) && <p className="afc-caution" data-testid="studio-af3-parameter-status">현재 파일은 성능 시험용 무작위 가중치로 판정되어 예측 실행이 차단됩니다.</p>}
            <AF3StageEvidence entry={active} live={live} checkedAt={checkedAt} />
            {blockers.length > 0 && <div className="afc-blockers" data-testid="studio-af3-blockers"><strong>{needsTrainedWeights ? "학습된 AF3 가중치 필요" : "실행을 막는 조건"}</strong><ul>{visibleBlockers.map((value, index) => <li key={index} data-testid={value === blockerTranslations[TEST_WEIGHT_BLOCKER] ? "studio-af3-parameter-status" : undefined}>{value}</li>)}</ul>{hasTranslatedBlockers && <details className="afc-raw-diagnostics" data-testid="studio-af3-raw-diagnostics"><summary>원문 진단 {blockers.length}개</summary><ul>{blockers.map((value, index) => <li key={index}>{value}</li>)}</ul></details>}</div>}
            {active.job.error && !repeatedJobError && <p className="afc-error" data-testid="studio-af3-job-error">{active.job.error}</p>}
            {active.output_validation?.status === "geometry_warning" && <p className="afc-caution" data-testid="studio-af3-quality-warning">출력 구조에 기하학적 품질 경고가 있습니다. 좌표와 경고를 검토하세요.</p>}
            {invalidOutput && <p className="afc-caution" data-testid="studio-af3-output-invalid">{active.output_validation?.status === "identity_failed" ? "출력에서 선택 성분·표적의 일치를 확인하지 못해 예측 구조 표시가 차단되었습니다." : "출력 검증을 완료하지 못했습니다. 계산 종료와 사용 가능한 예측 결과를 구분해야 합니다."}</p>}
            {active.output_validation?.status === "identity_verified_quality_unassessed" && <p className="afc-muted">출력의 성분·표적 일치는 확인했으며 구조 품질은 별도 평가가 필요합니다.</p>}
            {warnings.length > 0 && <details className="afc-warnings"><summary>실행·구조 해석 주의사항 {warnings.length}개</summary><ul>{warnings.map((value, index) => <li key={index}>{value}</li>)}</ul></details>}
            <div className="afc-job-actions">
              {active.job.status === "prepared" && <button className="afc-primary" onClick={() => void execute()} disabled={!!busy || !!refreshRequired || !!statusError || !active.readiness.runnable} data-testid="studio-af3-execute">{busy === "execute" ? <LoaderCircle size={17} className="spin" /> : <Play size={17} />}{busy === "execute" ? "계산 제출 중" : "이 성분 계산 시작"}</button>}
              {RETRY_STATUSES.has(active.job.status) && <button className="afc-primary" onClick={() => void prepare(true)} disabled={!!busy || !!refreshRequired} data-testid="studio-af3-retry">{busy === "retry" ? <LoaderCircle size={17} className="spin" /> : <RefreshCw size={17} />}같은 조건으로 다시 준비</button>}
              {active.job.status === "completed" && <button className="afc-primary" onClick={() => onResult(active.job.id, { smiles: compound.smiles, targetAccession })} data-testid="studio-af3-view-result">{invalidOutput ? "출력 검증 결과 확인" : "이 작업의 구조 보기"}<ArrowRight size={16} /></button>}
              <button className="afc-outline" onClick={() => void exportInput()} data-testid="studio-af3-input">입력 JSON</button>
            </div>
            {active.job.status === "prepared" && <small className="afc-muted">{active.readiness.runnable ? "실행 조건이 확인되었습니다. 계산 시작을 누르면 작업을 제출합니다." : "실행 조건을 충족한 뒤 입력을 다시 준비하세요."}</small>}
            {active.job.status === "completed" && <small className="afc-muted">완료된 작업의 성분·표적과 출력을 다시 확인한 뒤 위 뷰어에 표시합니다.</small>}
          </>}
        </div>
      </div>
      <AF3EvidencePanel entries={entries} active={active} onSelect={(id) => { setSelectedId(id); setLog(null); setNotice(null); }} />
      {notice && <p className="afc-notice" role="status">{notice}</p>}
      {statusError && <div className="afc-error" role="alert" data-testid="studio-af3-status-error"><CircleAlert size={17} /><span>{statusError}</span></div>}
      {error && <div className="afc-error" role="alert" data-testid="studio-af3-error"><CircleAlert size={17} /><span>{error}</span></div>}
      {active && <details className="afc-log"><summary><Terminal size={16} />최근 실행 로그{live && <span>자동 갱신</span>}</summary><div data-testid="studio-af3-log">{logError ? <p>로그 연결을 다시 확인하고 있습니다. 작업 상태는 로그와 별도로 갱신됩니다.</p> : log?.text ? <><pre>{logLines.slice(-160).join("\n")}</pre>{(log.truncated || logLines.length > 160) && <small>최근 로그 일부를 표시합니다. 전체 기록은 연구 기록의 run.log에서 확인하세요.</small>}</> : <p>아직 실행 로그가 없습니다. 실제 계산이 시작되면 이곳에 표시됩니다.</p>}</div></details>}
    </>}
  </section>;
}
